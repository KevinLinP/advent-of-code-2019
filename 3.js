...
const generateVisited = function (path) {
  ...
  path.forEach(function (movement) {
    const direction = movement[0];
    const distance = movement[1];
...
const visitedSets = paths.map(generateVisited);
...
const minDistance = intersections.reduce(function(currentMinDistance, intersection) {
  const distances = intersection.split(',').map(t => Math.abs(parseInt(t)));
  ...
  return (distance < currentMinDistance) ? distance : currentMinDistance;
}, Infinity);
