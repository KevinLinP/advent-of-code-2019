...
def split_lengths(wire):
    def split_length(length):
        return [length[0], length[1:]]
    return map(split_length, wire.split(','))

wires = map(split_lengths, wires)

def distance_grid(lengths):
    grid = {}
    x = 0
    y = 0
    wire_distance = 0
    ...
    for length in lengths:
        direction = length[0]
        distance = int(length[1])

        for n in range(distance):
            coordString = "{0},{1}".format(x, y)
            grid[coordString] = wire_distance
...
# def manhattan_distance(string):
    # distances = string.split(',')
    # distances = map(lambda s: abs(int(s)), distances)
    # return sum(distances)

def wire_distance(intersection):
    return wires[0][intersection] + wires[1][intersection]

distances = map(wire_distance, intersections)
print(min(distances))
